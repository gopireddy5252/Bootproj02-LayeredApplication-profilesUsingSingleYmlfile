...
public class EmployeeMYSQLDaoImpl implements IEmployeeDAO {
	private static final String GET_EMPS_BY_DESGS="select eno,sname,job,sal,deptno from empboot where job in(?,?,?)";
	private static final String INSERT_EMP_DETAILS="insert into empboot(eno,sname,job,sal,deptno)values(?,?,?,?,?)"; 
	@Autowired
